...
$id = $_REQUEST['id'];
$sql = sprintf("SELECT * FROM iba_post WHERE id = %d",
...
if (!empty($_POST)) {
    ...
    if (!empty($catchImgfileName)) {
      ...
      if($catchImgfileName != ""){
        ...
        $_SESSION['update']['catchy_img'] = $catchImgfileName;

        $updateUrl = $siteUrl.'/iba_admin/do-update.php';
...
<body>
<form id="frmUpdate" name="frmUpdate" method="post" action="" enctype="multipart/form-data">
  <dl>
    <dt>
...
?>
      <input name="created" type="text" id="date" size="20" maxlength="10" value="<?php echo (htmlspecialchars($data['created'],ENT_QUOTES));?>" />
    </dd>
    <dt>
      <label for="title">タイトル</label>
    </dt>
    <dd>
      <input name="title" type="text" id="title" size="50" maxlength="50" value="<?php echo (htmlspecialchars($data['title'],ENT_QUOTES));?>"/>
    </dd>
    <dt>
      <label for="catchy_img">Goiキャッチ画像</label>
    </dt>
    <dd>
      <p><img src="<?php echo (htmlspecialchars($siteUrl.'/blogImage/'.$data['catchy_img'],ENT_QUOTES));?>"></p>
...
?>
    ...
    <dd>
      <textarea name="content" id="mytextarea" rows="10" cols="100"/><?php echo (htmlspecialchars($data['content'],ENT_QUOTES));?></textarea>
      <div id="preview"></div>
    </dd>
    <dt>
    ...
    <dd>
      ...
      <input name="category" type="radio" class="cat" value="<?php printf(htmlspecialchars($categoryTable['category_name'])); ?>" <?php if($categoryTable['category_name'] == $data['category']){echo 'checked="checked"';} ?> />
  ...
  <div><a href="<?php echo $siteUrl.'/iba_admin/show-list.php'; ?>">一覧へ</a></div>
    <div><a href="<?php echo $siteUrl.'/iba_admin/uploadfile'; ?>">MEDIA</a></div>
    <div><a href="<?php echo $siteUrl.'/iba_admin/image-list.php'; ?>">写真を削除</a></div>
			<div id="confirm">
				...
				if(is_dir($dir) && $handle = opendir($dir)):
					while(($file = readdir($handle)) !== false): 
						...
						if($fileType == "file"):?>
							...
							<img src="<?php echo $siteUrl.'/iba_admin/'.$filePath; ?>" />
							</p>
						<?php endif; ?>
					<?php endwhile; ?>
				<?php endif; ?>
...
<iframe src="<?php echo $siteUrl.'/iba_admin/uploadfile'; ?>"></iframe>
			<div>
